...
def main(_):
    ...
    cluster_spec = {"ps": ps_hosts, "worker": worker_hosts, "master": master_host}
    ...
    if job_name == "ps":
        ...
        os.environ["TF_CONFIG"] = json.dumps(
            {"cluster": cluster_spec,
             "task": {"type": job_name, "index": task_index}, "environment": "cloud"})

        # Get configuration from environment variables.
        master_grpc = "grpc://"+(master_host[0] if job_name == "master" else worker_hosts[task_index])
        run_config = tf.contrib.learn.RunConfig(master=master_grpc,
                gpu_memory_fraction=0,
                model_dir=model_dir)
#        from tensorflow.contrib.learn.python.learn.estimators import run_config
#        run_config = run_config.RunConfig(master=master_grpc)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Hosts of master job. E.g. 'localhost:6662'"
    )
    parser.add_argument(
        "--job_name",
        type=str,
